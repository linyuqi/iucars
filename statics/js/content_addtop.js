...
/*多图上传2，SWF回调函数
*增加按分类、车系品牌保存图片 2015/7/8
*/
function change_images2(uploadid, returnid) {
    var d = uploadid.iframe.contentWindow;
    var in_content = d.$("#att-status").html().substring(1);
    var in_filename = d.$("#att-name").html().substring(1);
	var in_args = d.$("#att-args").html();
    var str = $('#' + returnid).html();
    var contents = in_content.split('|');
    var filenames = in_filename.split('|');
	var argsArr = in_args.split('|');
    $('#' + returnid + '_tips').css('display', 'none');
    if (contents == '') return true;
	if(filenames && argsArr){
		$.ajax({
			url:'/index.php?g=Products&c=Index&a=imgAdd',
			type:'post',
			data:{"filenames":contents,"args":argsArr},
